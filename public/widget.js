...
             target="_blank" rel="noopener noreferrer">
            ...
            ${/* 아이콘 URL이 있을 경우에만 아이콘 이미지 태그 추가 */''}
            ${p.decoration_icon_url ? `<img src="${p.decoration_icon_url}" alt="icon" class="prd_deco_icon" />` : ''}
...
  style.textContent = `
    ...
    .prd_deco_icon {
        ...
        top: 5px;
        right: 5px;
        width: 15%; /* 이미지 크기에 비례하여 조절 */
        max-width: 40px; /* 너무 커지지 않도록 최대 크기 지정 */
        height: auto;
        z-index: 10;
        pointer-events: none; /* 아이콘이 링크 클릭을 방해하지 않도록 함 */
    }
    /* ▲▲▲▲▲ [수정 3] 아이콘 스타일 추가 ▲▲▲▲▲ */
