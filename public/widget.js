;(function(){
  ...
  // ─── 페이지뷰/재방문 트래킹 ─────────────────────────────────────
  if (shouldTrack()) {
    track({ pageId, pageUrl: location.pathname, visitorId, type: 'view', device,
      referrer: document.referrer||'direct', timestamp: new Date().toISOString() });
  } else {
    track({ pageId, pageUrl: location.pathname, visitorId, type: 'revisit', device,
      referrer: document.referrer||'direct', timestamp: new Date().toISOString() });
  }
  ...
  document.body.addEventListener('click', e => {
    ...
    const payload = { pageId, pageUrl: location.pathname, visitorId,
      type:'click', element:elementType, device,
  ...
  function loadPanel(ul) {
    ...
    const cacheKey = ulDirect
      ? `direct_${ulDirect}`
      : (category ? `cat_${category}` : null);
    ...
    if (ulDirect) {
      ...
      .then(products => {
        if (storageKey) localStorage.setItem(storageKey, JSON.stringify(products));
  ...
  function renderProducts(ul, products, cols) {
    ...
    const items = products.map(p=>{
      const orig = p.price;
      const priceText = formatKRW(orig);
          ...
          <div class="prd_price"${couponText?' style="display:none;"':''}
            ...
            ${saleText
              ? `<span class="sale_price">${saleText}</span>
                 ${salePct>0?`<div class="sale_wrapper" style="display:inline-block;margin-right:4px;">
  ...
  .tabs_${pageId}{
    display:grid; gap:8px; max-width:800px;
    margin:16px auto;
