;(function(){
  ...
  // ─── 페이지뷰/재방문 트래킹 ─────────────────────────────────────
  if (shouldTrack()) {
    track({pageId, pageUrl:location.pathname, visitorId,
           type:'view', device,
  ...
  fetch(`${API_BASE}/api/${mallId}/events/${pageId}`)
    ...
    .then(ev => {
      ...
      document.querySelectorAll(`ul.main_Grid_${pageId}`).forEach(ul=>{
        ...
        loadMoreContainer.dataset.offset='0';
        ...
        function renderLoadMore(){
          ...
          btn.onclick=()=>{
            ...
            const offset=Number(loadMoreContainer.dataset.offset);
            fetch(
            ...
            .then(raw=>{
              ...
              loadMoreContainer.dataset.offset=offset+batchCount;
              renderLoadMore();
            })
            .catch(console.error);
          };
  ...
  function renderProducts(ul,products,cols){
    ...
    const html=products.map(p=>{
      const orig=p.price;
      const sale=p.sale_price!=null?`${p.sale_price.toLocaleString('ko-KR')}원`:null;
      const percent=sale?Math.round((orig-p.sale_price)/orig*100):null;
      return `
<li style="list-style:none;">
  ...
  <div class="prd_price"${sale?' style="display:none;"':''}
    ...
    ${sale?`<span class="sale_price">${sale}</span>
  ...
  <div class="coupon_wrapper" style="margin-top:4px;">
    <span class="prd_coupon_percent"
          style="color:#ff4d4f;font-weight:500;margin-right:4px;">
  ...
  .tabs_${pageId}{display:grid;gap:8px;max-width:800px;margin:16px auto;
    grid-template-columns:repeat(${tabCount},1fr);
  }
  .tabs_${pageId} button{padding:8px;font-size:16px;border:none;
    background:#f5f5f5;color:#333;cursor:pointer;border-radius:4px;
