;(function () {
    ...
    function invalidateProductCache() {
      try {
        ...
        console.info('[onimon.js] Product cache invalidated.');
      } catch (e) {
        console.warn('[onimon.js] invalidateProductCache error', e);
      }
    }
  ...
  async function loadPanel(ul) {
    ...
    } catch (e) { console.warn('[onimon.js] 캐시 파싱 오류', e); }
  ...
  // ✨ [수정] fetchProducts 함수에 아이콘 필드 요청 추가
  async function fetchProducts(directNosAttr, category, limit = 300) {
    ...
    // 5) 상품 렌더링
    // ────────────────────────────────────────────────────────────────
    // ✨ [수정] renderProducts 함수에 아이콘 렌더링 로직 추가
    function renderProducts(ul, products, cols) {
      ...
      const TAG_ICON_MAP = {}; // 상품 태그 기반 아이콘 (선택사항)
      ...
      ul.innerHTML = products.map(p => {
        ...
        if (salePriceNum != null && salePriceNum < finalPriceNum) finalPriceNum = salePriceNum;
        if (couponPriceNum != null && couponPriceNum < finalPriceNum) finalPriceNum = couponPriceNum;
        ...
        const mouseEvents = smallImg && mediumImg && smallImg !== mediumImg ? `onmouseover="this.querySelector('img').src='${smallImg}'" onmouseout="this.querySelector('img').src='${mediumImg}'"` : '';
    ...
    .prd_icons .prd_icon {
      height: 24px; width: auto; padding: 0;
    }
