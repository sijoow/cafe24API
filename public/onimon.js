...
      } else if (category) {
        // [MODIFIED] URL에 is_active=true 파라미터 추가 (기간 만료 시 404, 토큰 만료 시 409 반환 기대)
...
            // 카테고리/상품 API는 토큰 만료 시 409를 반환함 (백엔드 replyInstallGuard 로직)
...
        // 409 에러가 발생하면 상품 목록 대신 에러 메시지를 표시
        if (err.message && err.message.includes('409')) {
            console.error('상품 로드 실패: 토큰 만료로 인해 상품 노출이 차단되었습니다.');
            // 토큰 만료 시 빈 화면 유지 (또는 공지 메시지)
            return;
        }
        
        if (ul.parentNode) {
...
      try {
        // [MODIFIED] URL에 is_active=true 추가: 백엔드에서 기간 체크 및 토큰 체크를 트리거합니다.
        const response = await fetch(`${API_BASE}/api/${mallId}/events/${pageId}?is_active=true`);

        // [MODIFIED] 404 처리: 백엔드에서 기간 만료 시 404를 반환하면 노출을 차단합니다.
...
        
        // 토큰 만료 시에도 200 OK를 받지만, 이미 백엔드에서 이미지/상품 블록이 제거된 상태입니다.
...
        } else { // 구버전 데이터 처리
            // 토큰 만료 시 ev.images는 비어있으므로 이미지는 노출되지 않습니다.
