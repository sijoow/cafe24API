...
// ===================================
// ▼▼▼▼▼ 404 에러 해결을 위해 추가된 코드 ▼▼▼▼▼
// ===================================
// Products - 전체 목록
app.get('/api/:mallId/products', async (req, res) => {
  const { mallId } = req.params;
  try {
    // 페이지네이션을 위한 쿼리 파라미터 (limit, offset)
    const limit = parseInt(req.query.limit, 10) || 50;
    const offset = parseInt(req.query.offset, 10) || 0;
    const shop_no = 1; // 대부분의 경우 1번 상점

    // Cafe24 상품 목록 조회 API URL
    const url = `https://${mallId}.cafe24api.com/api/v2/admin/products`;

    // 작성하신 apiRequest 헬퍼 함수를 사용해 API 호출
    const { products = [] } = await apiRequest(mallId, 'GET', url, {}, {
      shop_no,
      limit,
      offset
    });

    // 조회된 상품 목록을 JSON 형태로 응답
    res.json(products);

  } catch (err) {
    console.error('[ALL PRODUCTS ERROR]', err);
    // 작성하신 replyInstallGuard 헬퍼 함수를 사용해 에러 처리
    return replyInstallGuard(res, err, '전체 상품 목록 조회 실패');
  }
});
// ===================================
// ▲▲▲▲▲ 여기까지 추가된 코드 ▲▲▲▲▲
