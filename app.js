...
// ================================================================
// ★★★★★★★★★★ [수정된 부분 1] Products - 전체 조회 (검색/페이징) ★★★★★★★★★★
// ================================================================
...
    const data = await apiRequest(mallId, 'GET', url, {}, params);
    ...
    if (products.length === 0) {
        return res.json({ products: [], total: data.total_count });
    }

    // 각 상품의 아이콘 정보를 가져오는 Promise 배열 생성
    const iconPromises = products.map(p =>
      apiRequest(mallId, 'GET', `https://${mallId}.cafe24api.com/api/v2/admin/products/${p.product_no}/icons`, {}, { shop_no })
        .then(iconData => ({
          product_no: p.product_no,
          icon_url: (iconData.icons && iconData.icons.length > 0) ? iconData.icons[0].image_path : null
        }))
        .catch(err => {
          // 개별 아이콘 조회 실패가 전체를 막지 않도록 처리
          console.error(`[GET ICONS FOR LIST ERROR] product ${p.product_no}:`, err.message);
          return { product_no: p.product_no, icon_url: null };
        })
    );

    // 모든 아이콘 정보를 병렬로 조회하고 결과를 받음
    const iconResults = await Promise.all(iconPromises);

    // product_no를 키로 하는 아이콘 URL 맵을 만들어 쉽게 찾을 수 있도록 함
    const iconMap = iconResults.reduce((map, item) => {
        map[item.product_no] = item.icon_url;
        return map;
    }, {});
...
      price: p.price,
      list_image: p.list_image,
      decoration_icon_url: iconMap[p.product_no] || null // 맵에서 해당 상품의 아이콘 URL을 찾아 추가
    }));
