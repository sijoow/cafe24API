...
app.get(['/', '/client'], async (req, res, next) => {
  try {
    ...
    // 설치 안되어 있으면 카페24 authorize로 리다이렉트
...
// 이제 static 파일 제공 (client 핸들러가 mall_id를 처리한 뒤에 둡니다)
...
// 설치 시작: mallId 기반 OAuth 권한 요청 (외부에서 직접 사용 가능)
...
// 콜백 핸들러: code → 토큰 발급 → DB에 mallId별 저장 → APP_URL 로 리다이렉트
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    // 설치 후 기본 루트로 리다이렉트 (필요시 카페24 내부 페이지로 변경 가능)
