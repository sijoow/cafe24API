...
const {
  ...
  CAFE24_API_VERSION,     // 예: '2023-06-01' (개발자센터에 명시된 최신 버전 사용)
  FRONTEND_URL,           // 예: https://onimon.shop
  BACKEND_URL,            // 예: https://port-0-...cloudtype.app
  CAFE24_SCOPES,          // dev center의 Permissions와 100% 동일 (콤마 구분)

  // --- 앱 삭제 콜백 검증 ---
  UNINSTALL_TOKEN,        // openssl rand -hex 32 등으로 생성한 랜덤 토큰
...
['MONGODB_URI','DB_NAME','CAFE24_CLIENT_ID','CAFE24_CLIENT_SECRET','CAFE24_API_VERSION','FRONTEND_URL','BACKEND_URL','CAFE24_SCOPES'].forEach(assertEnv);
...
app.all('/cafe24/uninstalled', async (req, res) => {
  try {
    ...
    // 2) mallId 추출 (Cafe24가 어떤 포맷으로 보내든 대비)
    const mallId =
